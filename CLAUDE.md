...
- **Prestação de contas (§15):** `eleicoes_prestacao_contas.json` — receitas + despesas pagas por candidato do roster, join por `SQ_CANDIDATO` com `consulta_cand_{ano}.zip`. **Sem dado real até o prazo da prestação parcial** (`PRESTACAO_PRAZO_PARCIAL` = 2026-09-13 no catálogo, curado em `eleicoes_contexto.json`) — até lá `n_candidatos = 0` por definição (não é falha), e a seção mostra contagem regressiva/"prazo encerrado" em vez de erro (`renderPrestacao()` distingue os dois estados por `Date.now()` vs. `prazo_parcial`). Único ponto do painel com cruzamento **publicamente possível** com Receita Federal: para doador pessoa jurídica (CNPJ), o TSE publica o nome declarado pela campanha ao lado do nome da própria RFB para aquele CNPJ (`NM_DOADOR`/`NM_DOADOR_RFB` — nome de coluna não confirmado); quando os dois existirem e divergirem, `collect_prestacao_contas.py` sinaliza (comparação exata por `casefold()`, não fuzzy) — para pessoa física (CPF) não há equivalente público (sigilo fiscal). Liga automaticamente no CI diário já existente; nenhum deploy novo será necessário quando o TSE publicar o primeiro dado real.
